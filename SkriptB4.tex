...
\begin{document}
	...
	\begin{lemma}\label{1.3.8}
		...
		\begin{proof}
			...
			\noindent Betrachte die durch Inklusion halbgeordnete Menge 
				\[X:=\set{P|P\ \mathrm{Untermodul von} \ M, P \subsetneq M} = \set{P|P\ \mathrm{Untermodul von} \ M, \set{x_1, \dots, x_n} \nsubseteq P}\]
	...
	\begin{definition}\label{1.4.8}
		Sei $M$ ein Modul. Dann heißt 
			\[\ell(M) := \sup \set{n \in \N_0| \mathrm{es\ gibt\ Untermoduln\ } M_0, \dots, M_n \mathrm{\ von\ } M \mathrm{\ mit\ } M_0 \supsetneq \cdots \supsetneq M_n} \in \N_0 \cup \set{\infty}\]
	...
	\begin{satz}\label{1.4.10}
		...
		\begin{proof}
			Man sieht sofort $\ell(M) = \sup \hat M, \ell(M/N) \overset{\ref{1.3.3}(a)}{=} \sup \hat{K}$ und $\ell(N) = \sup \hat N$ mit 
				\[\hat M := \set{m \in \N_0| \exists \mathrm{Untermoduln\ } M_0, \dots, M_m \mathrm{\ von\ } M: M = M_0 \supsetneq \cdots \supsetneq M_m = 0}\]
				\[\hat K := \set{k \in \N_0| \exists \mathrm{Untermoduln\ } L_0, \dots, L_k \mathrm{\ von\ } M: M = L_0 \supsetneq \cdots \supsetneq L_k = N}\]
				\[\hat N := \set{n \in \N_0| \exists \mathrm{Untermoduln\ } N_0, \dots, N_n \mathrm{\ von\ } M: N = N_0 \supsetneq \cdots \supsetneq N_n = 0}\]
	...
	\begin{satz}\label{1.4.13}
		...
		In diesem Fall ist die L\"ange einer jedem Kompositionsreihe von $M$ gleich der L\"ange von $M$.
