...
\begin{document}
	...
	\begin{def-satz}\label{2.1.10}
 	...
 	\end{lemma}
   	...
   	\begin{definition}\label{2.4.1}
   		...
   		Ist $\underline{v} = (v_1, \dots, v_n)$ eine beliebige Basis des $K$-Vektorraumes $L$ (insbesondere $n = [L:K]$) und $A = (a_{ij})_{1\leq i,j \leq n} = M(\varphi_a, \underline{v})$ die Darstellungsmatrix von $\varphi_a$ bez\"uglich $\underline{v}$ (also $av_j = \sum_{i=1}^{n} a_{ij}v_i$ f\"ur alle $j \in \set{1,\dots, n}$), so hei\ss t $\chi_{L|K}(a) := \chi_{\varphi(a)} = \det (A-XI_n) \in K[X]$ das \emph{charakteristische Polynom}\index{Dedekindringe@\textbf{Dedekindringe}!Charakteristisches Polynom},
   		$N_{L|K}(a) := \det(A) = \chi_{L|K}(0)$ die \emph{Norm}\index{Dedekindringe@\textbf{Dedekindringe}!Norm} und $\tr_{L|K}(a) := \tr A = \sum_{i=1}^n a_{ii} = (-1)^{n-1}\glqq\textrm{Koeffizient von } X^{n-1} \textrm{ in } \chi_{L|K}(a)\grqq$ die \emph{Spur}\index{Dedekindringe@\textbf{Dedekindringe}!Spur} von $A$ bez\"uglich $L|K$ [diese Begriffe h\"angen nicht von der Wahl der Basis $\underline{v}$ ab].
   	\end{definition}
	...
	\begin{definition}\label{2.4.8}
		Eine algebraische K\"orpererweiterung $L|K$ hei\ss t \emph{rein inseparabel}\index{Dedekindringe@\textbf{Dedekindringe}!Rein Inseparabel}, wenn kein $a \in L \setminus K$ separabel \"uber $K$ ist.
	\end{definition}
	...
	\begin{definition}\label{2.4.11}
		Sei $L|K$ eine endliche K\"orpererweiterung. Dann hei\ss t
			...
			der \emph{Inseparabilit\"atsgrad}\index{Dedekindringe@\textbf{Dedekindringe}!Inseparabilit\"atsgrad} von $L|K$.
	\end{definition}
