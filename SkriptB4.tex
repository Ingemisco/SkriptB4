...
\begin{document}
	...
	\begin{satz}\label{1.5.13} Satz von Krull-Remak-Schmidt\newline
		...
		\begin{proof}
			...
			\begin{tcolorbox}[colback=white,colframe=blue,arc=0cm]
				\textbf{Behauptung 1}: \begin{tikzcd}
	...
	\begin{def-satz}\label{2.1.10}\hfill\newline
 	...
 	\end{lemma}
   	...
   	\begin{definition}\label{2.4.1}\hfill\newline
   		...
   		das \emph{charakteristische Polynom}\index{Norm, Spur, Diskriminante@\textbf{Norm, Spur, Diskriminante}!Charakteristisches Polynom}, 
   		\[N_{L|K}(a) := \det(A) = \chi_{L|K}(0)\] 
   		die \emph{Norm}\index{Norm, Spur, Diskriminante@\textbf{Norm, Spur, Diskriminante}!Norm} und 
   		\[\displaystyle \tr_{L|K}(a) := \tr A = \sum_{i=1}^n a_{ii} = (-1)^{n-1}\glqq\textrm{Koeffizient von } X^{n-1} \textrm{ in } \chi_{L|K}(a)\grqq\] die \emph{Spur}\index{Norm, Spur, Diskriminante@\textbf{Norm, Spur, Diskriminante}!Spur} von $A$ bez\"uglich $L|K$ [diese Begriffe h\"angen nicht von der Wahl der Basis $\underline{v}$ ab].
   	\end{definition}
	...
	\begin{bemerkung}\label{3.2.1}\hfill\newline
		...
		\begin{enumerate}[(a)]
			...
			\begin{center}
				\begin{tikzcd}
					\Q[X]/(f) \rar{\cong}[swap]{\overline{X} \mapsto a} & K\\
					& & \mathcal{O}_K \ular[hook][description, sloped]{\supseteq} \ar[dd, two heads]\\
					\Z[X]/(f) \ar[uu, hook] \rar{\cong}[swap]{\overline{X} \mapsto a} \dar[two heads] & \Z[a] \urar[hook][description, sloped]{\subseteq} \dar[two heads]\\
					\F_p[X]/(\overline{f}) \rar{\cong}[swap]{\overline{X} \mapsto a} & \Z[a]/p\Z[a] \rar{\overset{\overset{p\nmid[\mathcal{O}_K:\Z[a]]}{\Downarrow}}{\cong}} & \mathcal{O}_K/p\mathcal{O}_K
				\end{tikzcd}
			\end{center}
 		\end{enumerate}
	\end{bemerkung}
 			...
 			\begin{enumerate}[(a)]
				...
				\end{flalign*}
 				...
 				\begin{flalign*}
 					...
 					&= \underbrace{p_1^{\alpha_1} \cdots p_m^{\alpha_m}}_{=n}\brac{1-\frac{1}{p_1}} \cdots \brac{1-\frac{1}{p_m}}.
 				\end{flalign*}
 			\end{enumerate}
 		\end{proof}
 	\end{proposition}
